SELECT [ld].[Lookback_ID], (SELECT [d].[Blood_Type] FROM [DONOR] [d] WHERE [d].[Donor_ID] = [ld].[Donor_ID]) AS [Blood_Type], [ld].[BUI], [ld].[Look_CMP_Code], 
    [ld].[Look_CMP_Desc], [ld].[Look_Ship_Date], [ld].[Look_Ship_ID], (SELECT [td].[Disease_ID] FROM [TRACKING_DATA] [td] WHERE [td].[BUI] = [ld].[Look_BUI]) AS [Disease_ID],
    (SELECT [td].[Disease_Desc] FROM [TRACKING_DATA] [td] WHERE [td].[BUI] = [ld].[Look_BUI]) AS [Disease_Desc],
    (SELECT [td].[Test_ID] FROM [TRACKING_DATA] [td] WHERE [td].[BUI] = [ld].[BUI]) AS [Test_ID], [ld].[Look_BUI], [ld].[Look_Ship_ID], [ld].[Look_Ship_Box_No],
    [ld].[Look_Loc_Code], [ld].[Look_Loc_Code], (SELECT [mo].[Org_Name] FROM [MED_ORG] [mo] WHERE [mo].[Org_ID] = [ld].[Org_ID]) AS [Org_Name],
