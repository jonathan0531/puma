SELECT [ld].[Lookback_ID], 
    ...
    (SELECT TOP 1 [td].[Test_ID] FROM [TRACKING_DATA] [td] WHERE [td].[BUI] = [ld].[Look_BUI]) AS [Test_ID], 
    ...
    (SELECT [dc].[Disease_Desc] FROM [DISEASE_CONFIG] [dc] WHERE [dc].[Disease_ID] = [ld].[Disease_ID]) AS [Disease_Desc],
    (SELECT [u].[Donation_Date] FROM [UNIT] [u] WHERE [u].[BUI] = [ld].[Look_BUI]) AS [Donation_Date], 
    [ld].[Look_BUI],
